...
Текущий MVP реализует первый вертикальный срез этого сценария: контроль ожидаемых ERP-документов в snapshot-области 1C, собранной от выбранной поставки. Поставка является точкой входа расследования, но не строгой границей принадлежности документов. Независимая двусторонняя population-сверка относится к следующей фазе.
...
- **HTML architecture view** - читаемая версия без пересекающихся линий: `C4_ARCHITECTURE.html`;
