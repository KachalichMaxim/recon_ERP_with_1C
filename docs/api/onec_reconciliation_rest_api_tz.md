...
2. Для `sales[]` нужно отдельно возвращать:
   - `tax_invoice_number` - номер связанного выданного счета-фактуры;
   - `tax_invoice_date` - дата связанного выданного счета-фактуры.

   Эти поля заполняются независимо от наличия счета покупателю. Счет покупателю является самостоятельным объектом `customer_invoices[]`; счет-фактура не заменяет его при отсутствии.
