...
- оплаты клиента - `get_paidsum` и распределения `veda_acchist_docs.f_clssum`;
...
- `MISSING_IN_1C` - документ есть в ERP, но отсутствует в 1C.
- `MISSING_IN_ERP` - документ есть в 1C, но отсутствует в ERP.
- `AMOUNT_MISMATCH` - расходится сумма.
- `DATE_MISMATCH` - расходится дата.
- `NUMBER_MISMATCH` - расходится номер.
- `CONTRACT_MISMATCH` - расходится договор/поставка.
...
- ячейки `№ спецификации`, `Сумма оплаты`, `Возмещаемые расходы`, `Невозмещаемые расходы`, `№ счф`, `(+/-)` объединяются на количество строк счетов поставки;
- все номера закрывающих документов внутри `№ счф` выводятся через перенос строки;
